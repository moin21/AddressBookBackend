...
public class AddressBookController {
     ...
     * Created responseDTO object with message and addContact method applied on
     * iAddressBookService.
     *
    ...
    public ResponseEntity<ResponseDTO> createContact(@Valid @RequestBody AddressBookDTO addressBookDTO) {
        ResponseDTO responseDTO = new ResponseDTO("Contact Created Successfully",
                iAddressBookService.addContact(addressBookDTO));
        return new ResponseEntity<>(responseDTO, HttpStatus.CREATED);
    }

    @GetMapping("/get-all")
    public ResponseEntity<ResponseDTO> getAllContacts() {
        ResponseDTO responseDTO = new ResponseDTO("Contact List Received Successfully",
                iAddressBookService.getAllContacts());
    ...
    public ResponseEntity<ResponseDTO> editAddress(@Valid @RequestBody AddressBookDTO employeeDTO,
            @PathVariable int id) {
        ResponseDTO responseDTO = new ResponseDTO("Employee Updated Successfully",
                iAddressBookService.editContact(employeeDTO, id));
        return new ResponseEntity<>(responseDTO, HttpStatus.OK);
    }
}
